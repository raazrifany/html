    ...
    <style>
        ...
        * {
        ...
        .checkout-button {
            ...
            cursor: pointer;
            margin-top: 20px;
            float: right; /* Mengatur posisi tombol ke kanan */
            font-weight: bold;
        }
        ...
        .total-all {
            ...
            margin-top: 20px; /* Tambahkan ruang atas */
            float: left; /* Mengatur posisi total price ke kiri */
        }
    </style>
    <title>Cart</title>
</head>
<body>
    <div class="navbar">
            ...
            <a style="float:right" class="img-menu"><img src="dummy/shop-cart-active.png" /></a>
    </div>
    ...
    <div class="cart-container">
        ...
        if ($countCart > 0) {
            ...
            while ($dataCart = mysqli_fetch_array($resultCart, MYSQLI_ASSOC)) {
                ...
                // Tambahkan total harga dari item saat ini ke total harga semua item
                $totalAll += $dataCart['totalharga_cart'];
        ?>
...
<!-- Tombol Checkout -->
        <form action="checkout.php" method="post"  target="_blank">
            <input type="hidden" name="produkArray" value='<?php echo json_encode($produkArray); ?>'>
        ...
        <!-- Total Price -->
        <div class="total-all">Total: Rp. <?php echo number_format($totalAll, 0, ',', '.'); ?></div>    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Tambahkan event listener untuk mendengarkan perubahan pada input jumlah produk
    ...
    function updateTotalPrice() {
        ...
        let totalAll = 0; // Variabel untuk menyimpan total harga semua produk
        totalPriceElements.forEach(function(element) {
            ...
            totalAll += totalPrice; // Tambahkan harga produk saat ini ke total harga semua produk
        });

        // Perbarui elemen dengan class 'total-all' untuk menampilkan total harga semua produk
        let totalAllElement = document.querySelector('.total-all');
        totalAllElement.innerText = 'Total: Rp. ' + totalAll.toLocaleString('id-ID');
    }
